...
(async () => {
	...
	// while((await page.$x("//a[contains(text(), \"Next page\")]")).length > 0) {
		...
		for(let i = 0; i < urls.length; i++) {
			...
			const {stdout, stderr} = await exec(`D: && wget --convert-links -e robots=off --adjust-extension --page-requisites --no-parent --no-clobber --restrict-file-names=windows -U \"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.77 Safari/537.36\" -P ${__dirname + "\\wiki"} ${page.url()}`);
